            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 flex-wrap mt-1">
                      {(repo.ciCd as string[]).map((ci) => (
                        <Badge key={ci} variant="outline">{ci}</Badge>
                      ))}
                    </div>
                  </div>
                )}
                {(repo.deployTargets as string[])?.length > 0 && (
                  <div>
                    <span className="text-sm font-medium">Deploy Targets: </span>
                    <div className="flex gap-1 flex-wrap mt-1">
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 flex-wrap mt-1">
                      {(repo.infraAsCode as string[]).map((iac) => (
                        <Badge key={iac} variant="outline">{iac}</Badge>
                      ))}
                    </div>
                  </div>
                )}
                    ...
                    <Sheet key={detector}>
                      <SheetTrigger asChild>
                        <Button variant="outline" size="sm" className="w-full justify-start">
                          ...
                          {detector} ({proofs.length} file{proofs.length > 1 ? 's' : ''})
                        </Button>
                      </SheetTrigger>
                      <SheetContent className="overflow-y-auto">
                        <SheetHeader>
                          ...
                          <SheetDescription>
                            Evidence from {proofs.length} file{proofs.length > 1 ? 's' : ''}
                          </SheetDescription>
                        </SheetHeader>
